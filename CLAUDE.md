...
- `15 5 * * *` — `sendvery:auth:purge-magic-links` (drop magic-link tokens past their 30-day forensic retention; a token row is written for every sign-in attempt — including every address a signup-abuse bot submits — and nothing else deletes them)
