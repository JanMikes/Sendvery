...
final class OnboardingTest extends WebTestCase
{
    ...
    #[Test]
    public function onboardingIngestionVerifyEndpointReturns200(): void
    {
        $client = self::createClient();
        $user = $this->createNewUserWithTeam(teamStepCompleted: true, withDomain: true);
        ...
        self::assertResponseIsSuccessful();
        $body = (string) $client->getResponse()->getContent();
        self::assertStringContainsString('dmarc-verify', $body);
    }

    #[Test]
    public function onboardingIngestionVerifyRedirectsToDomainWhenNoDomain(): void
    {
        $client = self::createClient();
        $user = $this->createNewUserWithTeam(teamStepCompleted: true);
    ...
    #[Test]
    public function onboardingIngestionVerifyRedirectsToDashboardWhenAlreadyCompleted(): void
    {
        $client = self::createClient();
        $user = $this->createCompletedUser();
    ...
    #[Test]
    public function onboardingCompleteNeverShowsVerificationWarning(): void
    {
        // Step 4 must never contradict step 3 or show a verification verdict.
        // Verification banners live on the dashboard where the daily cron settles them.
        $client = self::createClient();
        $user = $this->createNewUserWithTeam(teamStepCompleted: true, withDomain: true);
        ...
        self::assertStringNotContainsString('No reports yet', $body);
    }
