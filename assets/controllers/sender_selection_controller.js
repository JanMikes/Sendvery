...
/*
 * Drives the per-row checkbox + sticky toolbar UX on the Sender Inventory.
 *
 * Targets:
...
export default class extends Controller {
    ...
    updateCount() {
        const checked = this.element.querySelectorAll('input[type=checkbox][name="senderIds[]"]:checked');
    ...
    clearAll() {
        this.element.querySelectorAll('input[type=checkbox][name="senderIds[]"]').forEach((cb) => {
            cb.checked = false;
        });
        this.updateCount();
    }
