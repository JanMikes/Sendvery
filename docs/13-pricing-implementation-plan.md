...
| 6 — Cutover | ✅ done (code-side) | 2026-05-22. Code-side cutover is in place: PricingTable CTAs flip from `request_beta_access` to `dashboard_billing_upgrade` based on `SENDVERY_STRIPE_LIVE` env (default `false`); AI CTAs stay on lead capture per DEC-057. `sendvery:beta-leads:launch-announce` one-shot command sends launch-announcement emails to fake-door leads (supports `--since`, `--coupon`, `--dry-run`). **Human actions still required:** create Stripe products + 12 prices, set live env vars, flip `SENDVERY_STRIPE_LIVE=true`, run the announce command. |
