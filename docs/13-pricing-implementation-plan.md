...
After Phase 7, a follow-up commit replaced env-flag-based gating with key-presence checks and deprovisioned the fake-door entirely:
...
- Critical fixes from final review: `PlanEnforcement::ensureCurrentPeriod` now uses `INSERT ... ON CONFLICT DO NOTHING` (was racy under concurrent first-of-period workers); `UpgradeTeamPlanHandler` no longer overwrites `stripeCustomerId` with `''` when the webhook payload omits the field.

**Known follow-up:** the `canParseReport` check + the increment inside `ProcessDmarcReportHandler` are not atomic — a Free team could land slightly over the 100-reports/mo cap under bursty parallel ingestion. Worth refactoring to an atomic `UPDATE ... WHERE count < :max RETURNING ...` once we see real traffic at the cap.

### What landed in Phase 7 (2026-05-22)

- `src/Command/WarnApproachingPlanLimitsCommand.php` — `sendvery:plan-limits:warn-approaching` cron. Iterates teams whose `plan_warning_at` is null, computes %used for domains/seats/reports/AI, emails the owner via `TeamMembershipRepository::findOwnerForTeam` when any cap crosses 80%. Stamps `plan_warning_at = now` to dedupe; Upgrade/Downgrade/SetTeamPlan handlers clear that timestamp on plan change so a new plan window gets a fresh warning.
- Sentry breadcrumbs: `SubscriptionManager::dispatchStripeEvent` (category `stripe.webhook` on every event, with team/plan/interval metadata), `PlanGatedAiInsightsService` (category `plan.ai_quota_exceeded` when throwing AiQuotaExceeded), `ProcessReceivedReportEmailHandler` (category `plan.report_cap_hit` when an inbound report gets quarantined for over-cap).
- `CLAUDE.md` crontab list updated with `0 8 * * * sendvery:plan-limits:warn-approaching`.
- Tests: `WarnApproachingPlanLimitsCommandTest` covers no-action, domain 80%, monthly-report 80%, and the dedupe-via-plan_warning_at case. **1126 tests green** (up from 1122).

**Deferred to future iterations** (build when there's signal to measure): daily admin "subscription health" email, admin-only active-subscriptions dashboard, AI cost monitoring per team. None are blocking the cutover.
...
- `src/Services/Stripe/SubscriptionManager.php` — new `updateSubscription(team, plan, interval)` method calls Stripe's `subscriptions.update` with `proration_behavior: create_prorations`. Refactored webhook routing into a public `dispatchStripeEvent(eventType, data)` method (handleWebhook still validates signature first). New handlers for `customer.subscription.updated` (dispatches `UpgradeTeamPlan` based on metadata) and `invoice.payment_failed` (log-only — Stripe's retry schedule + the subsequent `customer.subscription.deleted` event handle the downgrade).
- `src/Controller/Dashboard/UpgradePlanController.php` — branches on `team.stripeSubscriptionId`: existing subscribers get an in-place `updateSubscription`, new conversions get a fresh Checkout session. AI-not-yet-purchasable still redirects to lead capture with `source=dashboard-ai-curious`.
- `src/Controller/Dashboard/BillingController.php` — looks up AI quota usage when the plan has AI and passes it into the template.
- `templates/dashboard/billing.html.twig` — adds an AI quota progress bar for AI-enabled plans ("X of Y on-demand explanations this month").
- Tests: `SubscriptionManagerTest` covers `dispatchStripeEvent` for `checkout.session.completed`, `customer.subscription.updated`, `customer.subscription.deleted`, `invoice.payment_failed`, unknown events, and the no-metadata short-circuit. **1119 tests green** (up from 1113).

### What landed in Phase 4 (2026-05-22)

- `assets/controllers/pricing_controller.js` — new Stimulus controller. Targets: card, price, strikethrough, billingNote, savingsChip, aiFeature, cta, aiToggle, billingButton. Reads/writes `localStorage["sendvery_pricing"]`. Cards use dataset attributes (`data-price-annual`, `data-price-ai-annual`, etc.) so the swap is data-driven with no per-card branching.
- `templates/components/PricingTable.html.twig` — full rewrite. 4 cards (Free / Personal / Pro / Business) + cadence pill + AI toggle + "Talk to us about Enterprise" line. Server-renders the annual + no-AI state for non-JS users. AI-curious CTAs swap to `request_beta_access?source=pricing-ai-curious`. The "Most popular" badge stays on Personal across all toggle states.
- Tests: new `PricingTableTest` covers card count, server-rendered annual default, all (plan, cadence, AI) price data attributes, AI-not-available marker on Free, presence of both toggles. **1113 tests green** (up from 1107).
